...
import PegaExtensionsMyForm from './index';
...
const meta: Meta<typeof PegaExtensionsMyForm> = {
  title: 'PegaExtensionsMyForm',
  component: PegaExtensionsMyForm,
  excludeStories: /.*Data$/,
  parameters: {
    type: 'Form'
  }
...
export default meta;
type Story = StoryObj<typeof PegaExtensionsMyForm>;
...
export const BasePegaExtensionsMyForm: Story = args => {
  const props = {
    getPConnect: () => {
      return {
        getChildren: () => {
  ...
  return (
    <>
      <PegaExtensionsMyForm {...props} {...args}>{regionAChildren}</PegaExtensionsMyForm>
    </>
  );
};

BasePegaExtensionsMyForm.args = {
  showLabel: true,
  label: 'Form template',
  NumCols: 1,
};
